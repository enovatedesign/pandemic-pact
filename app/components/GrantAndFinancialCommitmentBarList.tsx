...
export function GrantAndFinancialCommitmentBarList({
    ...
    const maxTotalNumberOfGrants = Math.max(...data.map((data: any) => data["Total Grants"]))
    const maxAmountCommitted = Math.max(...data.map((data: any) => data["Known Financial Commitments (USD)"]))
                ...
                {data.map((datum: any) => (
                    <Fragment key={datum["Category Value"]}>
                        <div className="self-center mt-1 col-span-4 first:mt-0">
                            <p className="text-gray-600 text-sm">{datum["Category Label"]}</p>
                        </div>
                        ...
                        <div className="self-center pl-2 col-span-1 md:col-span-1 justify-self-end">
                            <p className="text-xs text-gray-600">{dollarValueFormatter(datum["Known Financial Commitments (USD)"])}</p>
                        </div>
                    </Fragment >
                ))}
            </div>
